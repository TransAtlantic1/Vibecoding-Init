...
SCRIPT_NAME="init-codex-fj.sh"
LOG_TAG="init-codex"
BEGIN_MARKER="# >>> codex init >>>"
END_MARKER="# <<< codex init <<<"
...
DEFAULT_ENV_KEY_NAME="CODEX_API_KEY"
...
upsert_profile_block() {
...

  {
    ...
    printf "export CODEX_ENV_FILE='%s'\n" "$escaped_env_file"
    printf "%s() (\n" "$alias_name"
    printf "  export CODEX_HOME='%s'\n" "$escaped_home"
    printf "  if [[ -f '%s' ]]; then\n" "$escaped_env_file"
    printf "    set -a\n"
...
write_config_file() {
  ...
  local provider_name="codex_provider"
