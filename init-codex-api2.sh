...
SCRIPT_NAME="init-codex-api2.sh"
LOG_TAG="init-codex-api2"
BEGIN_MARKER="# >>> codex-api2 init >>>"
END_MARKER="# <<< codex-api2 init <<<"

DEFAULT_API_KEY=""
DEFAULT_BASE_URL="https://capi.quan2go.com/openai"
DEFAULT_MODEL="gpt-5.5"
DEFAULT_CODEX_HOME_ROOT="${WORKSPACE_ROOT}/.codex_home_api2"
DEFAULT_ALIAS="codex-api2"
DEFAULT_H1_PROXY_ENABLED="true"
DEFAULT_H1_PROXY_PORT="8787"
DEFAULT_H1_PROXY_CONF="/etc/nginx/conf.d/codex_h1_proxy.conf"
DEFAULT_ENV_FILE="${SCRIPT_DIR}/.env"
DEFAULT_ENV_KEY_NAME="CODEX_API2_API_KEY"
...
Options:
  ...
  --codex-home-root <dir>  Root dir for CODEX_HOME (default: \$HOME/fj/.codex_home).
  ...
  --check-connectivity     Run a raw HTTP probe and a Codex CLI smoke test after setup.
...
upsert_profile_block() {
...

  {
    cat "$tmp_file"
    printf '\n%s\n' "$BEGIN_MARKER"
    printf "export CODEX_API2_HOME='%s'\n" "$escaped_home"
    printf "export CODEX_API2_ENV_FILE='%s'\n" "$escaped_env_file"
    printf "%s() (\n" "$alias_name"
...
write_config_file() {
  ...
  local provider_name="codex_api2_provider"
...
probe_responses_endpoint() {
  local probe_url="${1%/}/responses"
  local probe_body
  local status_code
  local body

  if ! command -v curl >/dev/null 2>&1; then
    warn "Skipping raw HTTP probe because curl is not installed."
    return
  fi
  ...
  status_code="$(
    curl -sS -o "$probe_body" -w '%{http_code}' \
      ...
      -H 'Authorization: Bearer sk-probe' \
      -X POST "$probe_url" \
      --data "{\"model\":\"${MODEL}\",\"input\":\"ping\"}" || true
  )"
  body="$(tr -d '\r' < "$probe_body")"
  rm -f "$probe_body"

  case "$status_code" in
    400)
      if [[ "$body" == *"not a codex request"* ]]; then
        log "Raw HTTP probe reached ${probe_url}; upstream recognized the Codex responses route."
      else
        warn "Raw HTTP probe returned 400 from ${probe_url}: ${body:-<empty>}"
      fi
      ;;
    401|403)
      log "Raw HTTP probe reached ${probe_url}; upstream requires a valid key."
      ;;
    404)
      warn "Raw HTTP probe returned 404 from ${probe_url}. Check --base-url."
      ;;
    000)
      warn "Raw HTTP probe could not reach ${probe_url}. Check DNS, network, or proxy."
      ;;
    *)
      warn "Raw HTTP probe returned ${status_code} from ${probe_url}: ${body:-<empty>}"
      ;;
  esac
}
...
run_codex_smoke_test() {
  ...
  if command -v timeout >/dev/null 2>&1; then
    if env CODEX_HOME="$FINAL_CODEX_HOME" OPENAI_API_KEY="$API_KEY" \
      ...
      if grep -qi 'user' "$log_file"; then
        warn "The upstream error looks user/account related. Re-check whether ${ENV_KEY_NAME} belongs to an enabled account on this provider."
      fi
    fi
  else
    if env CODEX_HOME="$FINAL_CODEX_HOME" OPENAI_API_KEY="$API_KEY" \
...
if [[ -z "$_ALIAS_FROM_CLI" ]] && [[ -n "${VIBECODING_ALIAS_CODEX_API2:-}" ]]; then
  ALIAS_NAME="${VIBECODING_ALIAS_CODEX_API2}"
fi
...
if [[ "$SELF_TEST_ENABLED" == "true" ]]; then
  probe_responses_endpoint "$EFFECTIVE_BASE_URL"
  run_codex_smoke_test
fi
