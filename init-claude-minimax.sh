...
SCRIPT_NAME="init-claude-minimax.sh"
LOG_TAG="init-claude-minimax"
BEGIN_MARKER="# >>> claude-minimax init >>>"
END_MARKER="# <<< claude-minimax init <<<"

DEFAULT_CLAUDE_HOME_ROOT="${WORKSPACE_ROOT}/.claude_home_minimax"
DEFAULT_ALIAS="claude-minimax"
...
upsert_profile_block() {
...

  {
    cat "$tmp_file"
    printf '\n%s\n' "$BEGIN_MARKER"
    printf "export CLAUDE_MINIMAX_HOME='%s'\n" "$escaped_home"
    printf "export CLAUDE_MINIMAX_ENV_FILE='%s'\n" "$escaped_env_file"
    printf "%s() (\n" "$alias_name"
...
if [[ -n "${VIBECODING_ALIAS_CLAUDE_MINIMAX:-}" ]]; then
  ALIAS_NAME="${VIBECODING_ALIAS_CLAUDE_MINIMAX}"
fi
