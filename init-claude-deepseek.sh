...
SCRIPT_NAME="init-claude-deepseek.sh"
LOG_TAG="init-claude-deepseek"
BEGIN_MARKER="# >>> claude-deepseek init >>>"
END_MARKER="# <<< claude-deepseek init <<<"

DEFAULT_AUTH_TOKEN=""
DEFAULT_BASE_URL="https://api.deepseek.com/anthropic"
DEFAULT_CLAUDE_HOME_ROOT="${WORKSPACE_ROOT}/.claude_home_deepseek"
DEFAULT_ALIAS="claude-deepseek"
DEFAULT_ENV_FILE="${SCRIPT_DIR}/.env"
DEFAULT_ENV_KEY_NAME="DEEPSEEK_API_KEY"
...
Options:
  --auth-token <token>        DeepSeek API key (optional, defaults to ${ENV_KEY_NAME} in ${ENV_FILE}).
  --base-url <url>            Anthropic-compatible base URL (default: ${DEFAULT_BASE_URL}).
  --claude-home-root <dir>    Root dir for isolated Claude HOME (default: ${DEFAULT_CLAUDE_HOME_ROOT}).
...
upsert_profile_block() {
...

  {
    cat "$tmp_file"
    printf '\n%s\n' "$BEGIN_MARKER"
    printf "export CLAUDE_DEEPSEEK_HOME='%s'\n" "$escaped_home"
    printf "export CLAUDE_DEEPSEEK_ENV_FILE='%s'\n" "$escaped_env_file"
    printf "%s() (\n" "$alias_name"
    ...
    printf "  exec claude --settings '%s' --effort max \"\$@\"\n" "$escaped_settings"
    printf ")\n"
    printf '%s\n' "$END_MARKER"
  } >"$profile_file"
...
if [[ -z "$_ALIAS_FROM_CLI" ]] && [[ -n "${VIBECODING_ALIAS_CLAUDE_DEEPSEEK:-}" ]]; then
  ALIAS_NAME="${VIBECODING_ALIAS_CLAUDE_DEEPSEEK}"
fi
